...
- **캠페인명에는 `_C##` 코드를 붙일 것 (네이버·구글·다음 전부).** 키워드 탭의 보종은 캠페인명의 `_C##` →
  `CAMP_CAT_MAP` 으로 정해진다. 코드가 없으면 `core.js` 의 `guC()` 가 캠페인명 문자열로 보종을 추측하는데,
  추측 규칙이 6개 보종만 커버해서 신생아·치매·자동차·저축성·생명·종신·정기·주택화재는 '보험'으로 잘못 분류된다.
  네이버 행의 키는 `키워드||매체||기기` 라 보종을 키에 넣지 않는다(CRM엔 캠페인 정보가 없어 DB를 붙일 수 없게 된다) —
  한 키워드가 캠페인 둘에 걸치면 광고비가 큰 쪽의 보종을 따른다
